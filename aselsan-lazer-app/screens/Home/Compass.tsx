...
function Compass({ navigation }: any) {
    ...
    React.useCallback(() => {
      ...
      let listener = DeviceEventEmitter.addListener(
        ...
        ({
          ...
          setDistanceUnit(
            distanceConversion(0, distance_unit, distance_unit).unit
          );
          ...
          Animated.timing(spinValue, {
            toValue:
              360 -
              angleConversion(azimuth, angle_unit, AngleUnitTypes.Derece.id)
                .angle,
            duration: 1000,
        ...
        {distance.map((val: any, index: number) => (
          <CView key={index} padding="4 0" center>
            <FontText
              ...
              bold
              size={6 / (row ? distance.length * 0.8 : 1)}
            />
            {distance.length > 1 ? (
              <FontText
                bold
  ...
  return (
    <CView flex={1} color="darkGray">
      <Swiper
        paginationStyle={{ bottom: -hp(6) }}
        index={MainStore.settings.home_screen_type}
        showsButtons
        loop={false}
        autoplay={false}
      >
        <CView
          key={HomeScreenTypes.MesafeVePusula.id}
          center
          color={"darkGray"}
          vertical="center"
          flex={1}
