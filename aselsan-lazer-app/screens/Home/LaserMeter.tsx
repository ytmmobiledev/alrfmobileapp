...
function LaserMeter({ navigation }: any) {
    ...
    async function getLocation() {
        const {coords} = await Location.getCurrentPositionAsync({ accuracy: Location.Accuracy.Highest, timeInterval: 10000, distanceInterval: 1 });
        _location={
            ...coords,
        ...
        React.useCallback(() => {
            let listener = DeviceEventEmitter.addListener("distance_and_compass",({distance,distance_unit,angle_unit,azimuth,elevation,roll}:any)=>{
                ...
                setTimeout(()=>{
                    try {
                        if(!_compass){
                            map.current.fitToCoordinates([...target_locations.map(({latitude,longitude})=>({latitude,longitude})),{latitude: _location.latitude, longitude: _location.longitude}],{edgePadding: { top: hp(10), right: hp(3), bottom: hp(20), left: hp(3) },animated:true})
    ...
    return (
        <Container>
            ...
            <CView flex={1}>
                    ...
                    }
                    {
                        target_len?
                            targets.map(({latitude,longitude}:any,index:number)=>
                                ...
                                >
                                    <CView center flex={1}>
                                        <CImage background style={{justifyContent:'center',alignItems:'center'}} source={target_len>1?require("../../assets/images/placeholder-full.png"):require("../../assets/images/placeholder.png")} orgHeight={30} orgWidth={30} resizeMode="stretch" >
                                            {
                                                target_len>1?
                                                    <Text  style={{bottom:5,fontSize:15,color:Colors.white,fontWeight:'bold'}}>{index+1}</Text>:null
                                            }
                                        </CImage>
                                    </CView>
                                </Marker>
                ...
                <CView vertical={"flex-end"} style={{flex:1, position:'absolute',bottom:hp(1),right:hp(1),zIndex:9999}}>
                        ...
                        onPress={()=>{
                            ...
                            try {
                                if(_compass){
                                    ...
                                    map.current.fitToCoordinates([...targets.map(({latitude,longitude})=>({latitude,longitude})),{latitude: location.latitude, longitude: location.longitude}],{edgePadding: { top: hp(10), right: hp(3), bottom: hp(20), left: hp(3) },animated:true})
                                }
            ...
            <CView style={{flex:1}} color="darkGray" center padding="0 0 0 10">
                    ...
                    loading?
                        ...
                        target_len?
                            <CView flex={1}>
                                <CView row center >
                                    {
                                        target_len>1?targets.map((_:any,index:number)=>
                                            <CView
                                                onPress={()=>{
                                                ...
                                                style={{zIndex:999}}
                                                padding="2" key={index} flex={1} row center>
                                                <FontText underline title={string.mesafe2+" "+(index+1)} size={select_target==index?2:1.7} bold color={select_target==index?"white":"text"} />
                                            </CView>
                                        ):<CView height={hp(5)}/>
                                    }
                                </CView>
                                <Swiper
                                    ...
                                    onIndexChanged={(index)=>{
                                        try {
                                            ...
                                            map.current.animateCamera({center:{latitude: target.latitude, longitude: target.longitude},pitch:0},{duration:500})
                                            setSelectTarget(index)
                                        }catch (e) {}
                                    }}
                                >
                                    {
                                        target_len?targets.map((target:any,index:number)=>
                                            <CView key={index} flex={1}  center>

                                                <CView flex={1}  width={"90%"} margin="1 2" padding="2" style={{alignSelf:'center', borderRadius:20}} colorHex={Colors.primary+"20"}>
                                                   <CView row vertical="center" horizontal="space-around">
                                                       <CView center>
                                                           <CImage source={require("../../assets/images/distance.png")} width={4} height={4} />
                                                           <CView row center>
                                                               <FontText position="center" title={parseFloat(target.distance).toFixed(2)+distance_unit.unit} size={1.8} color="white" bold />
                                                           </CView>
                                                           <FontText position="center" title={string.mesafe2+"\n"} size={1.4} color="primary" bold/>
                                                       </CView>
                                                       <CView center>
                                                           <CImage source={require("../../assets/images/target.png")} width={4} height={4} />
                                                           <CView row center>
                                                               <FontText position="center" title={"±"+parseFloat(distanceConversion(location.accuracy,DistanceUnitTypes.Metre.id, distance_unit.id).distance).toFixed(2)+distance_unit.unit} size={1.8} color="white" bold />
                                                           </CView>
                                                           <FontText position="center" title={"konumdogrulugu"} size={1.4} color="primary" bold/>
                                                       </CView>
                                                       <CView center>
                                                           <CImage source={require("../../assets/images/height.png")} width={4} height={4} />
                                                           <CView row center>
                                                               <FontText position="center" title={parseFloat(target.height+location.altitude??0).toFixed(2)+distance_unit.unit} size={1.8} color="white" bold />
                                                               {location.altitudeAccuracy?<FontText position="center" title={" ±"+parseInt(distanceConversion(location.altitudeAccuracy,DistanceUnitTypes.Metre.id, distance_unit.id).distance)} size={1.5} color="white" bold />:null}
                                                           </CView>
                                                           <FontText position="center" title={string.yukseklik+"\n"} size={1.4} color="primary" bold/>
                                                       </CView>
                                                ...
                                                <CView flex={1} width="90%" margin="1" row vertical="center" horizontal="space-around">
                                                    ...
                                                    <CView flex={1} center>
                                                        <CImage source={require("../../assets/images/placeholder.png")} width={3} height={3} />
                                                        {
                                                            target?.latitude && target?.longitude?
                                                                <CView  center>
                                                                    <FontText position="center" title={ConvertDDToDMS(target.latitude)+" N"} size={1.6} color="white" bold />
                                                                    <FontText position="center" title={ConvertDDToDMS(target.longitude)+" E"} size={1.6} color="white" bold />
                                                                </CView>:null
                                                        }
                                                    </CView>
                                                </CView>
                ...
                {
                    !loading?
                        <CView width="90%" vertical="center" horizontal="center" margin="0 0 0 1">
                            <CView
                                onPress={controlDevice}
                                padding="1 2" radius={20} row center color="primary"
                            >

                                <FontText position="center" title={"atisyap"} size={1.8} color="white"  bold/>
                                <CImage source={require("../../assets/images/measure.png")} resizeMode="contain" width={2.5} height={2.5} />
