...
const HomeScreenType = observer(() => {
  ...
  return (
    <CView>
        ...
        {Object.values(HomeScreenTypes).map(({ id, value }: any) => (
          ...
          >
            ...
            >
              ...
              <Feather
                name={home_screen_type == id ? "check-circle" : "circle"}
                size={hp(2.2)}
                color={Colors.text}
              />
            </CView>
      ...
      <FontText
        onPress={() => IStore.setCustomModal({ visible: false })}
        position="center"
        title={"kapat"}
        size={2}
        bold
        ...
        <CView>
          <Accordion
            ...
            renderHeader={({ title }, index) => (
              <CView padding="1.3">
                <CView
                  ...
                  width="100%"
                >
                  <FontText title={title} size={1.6} bold color={"white"} />
                  <AntDesign
                    name={active == index ? "up" : "down"}
                    size={hp(2)}
                    color={Colors.text}
                  />
                </CView>
            ...
            renderContent={({ content }, index) => (
              <CView padding="1.3 1.3 0 2" width="100%">
                <FontText title={content} size={1.5} color={"white"} />
              </CView>
            )}
            onChange={([index]) => {
              setActive(index);
    ...
    <CView>
      ...
      <CView padding="1 0">
        {Object.values(contacts).map(
          ({ title, value, onPress }: any, index: number) => (
            <CView
              key={index}
              onPress={() => {
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: Colors.darkGray }}>
      <CView width={"100%"} row vertical="center" horizontal="space-between">
        ...
        <CImage
          ...
          height={11.6}
          resizeMode={"stretch"}
          source={require("../../assets/images/secili-dahafazla.png")}
        />
      </CView>
      <Content>
        <CView vertical="center">
          ...
          >
            ...
            <CView row center>
              <FontText
                title={findType(HomeScreenTypes, home_screen_type)}
                size={1.4}
                color={"text"}
              />
              <MaterialIcons
